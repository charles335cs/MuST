...
def main():
    ...
    new_run_parser.add_argument('--data_dir', '-d', default='your_data_dir', type=str,
                                help='The directory where the data is stored.')
    new_run_parser.add_argument('--batch_size_background', '-bg', default=1, type=int, help='The batch size.')
    new_run_parser.add_argument('--batch_size_image', '-im', default=8, type=int, help='The batch size.')
    new_run_parser.add_argument('--resize_bound', '-r', default='0.3 0.4', type=str, help='The bound of image resize.')
    ...
    this_run_folder = utils.create_folder_for_run(train_options.runs_folder, 'train')
    writer = None
    ...
    # train from scratch: train encoder and decoder
    train(model , device , FWM_config , train_options , this_run_folder , writer)
    # start from scratch: train localization
    train_loc(model,device,FWM_config,train_options,this_run_folder,writer)

    # continue training: load checkpoint and train
    # checkpoint = torch.load("")
    # utils.load_from_checkpoint(model,checkpoint)
    # train(model , device , FWM_config , train_options , this_run_folder , writer)
    # train_loc(model , device , FWM_config , train_options , this_run_folder , writer)
